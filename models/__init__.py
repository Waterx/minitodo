...
mongua = MongoClient(host='127.0.0.1', port=27017)
'''https://www.cnblogs.com/LS1314/p/8609432.html'''
...
class Model(object):
    ...
    def _find(cls, **kwargs):
        ...
        mongo 数据查询
        """
        log('__init__', '_find', 'kwargs', kwargs)
        ...
        log('__init__', '_find', 'sort', sort)

        ds = mongua.zzdb[name].find(kwargs)
        log('__init__', '_find', 'ds', ds)

        if sort is not None:
            ds = ds.sort(sort)
        l = [cls._new_with_bson(d) for d in ds]
        ''''''
        log('__init__', '_find', 'l', l)
        return l

    @classmethod
    def _find_raw(cls, **kwargs):
        name = cls.__name__
        ds = mongua.zzdb[name]._find(kwargs)
        l = [d for d in ds]
        return l
        # 直接 list() 就好了
        # return list(l)
    ...
    def save(self):
        ...
        mongua.zzdb[name].save(self.__dict__)
        log("save", self)
        '''这里是新增id属性方便后续使用，因为_id是Objectid属性不方便用'''
    ...
    def delete(self):
        ...
        mongua.zzdb[name].update_one(query, values)
        # self.deleted = True
        # self.save()
    ...
    def data_count(self, cls):
        ...
        count = mongua.zzdb[name]._find(query).count()
        return count
